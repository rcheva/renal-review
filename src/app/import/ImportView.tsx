...
export default function ImportView() {
  ...
  const handleParse = () => {
    ...
    for (const block of blocks) {
      ...
      const qMatch = block.match(
        /(?:\*\*[Qq](?:uestion)?:\*\*|[Qq](?:uestion)?:\s*)\s*(.+)/
      );
      const aMatch = block.match(
        /(?:\*\*[Aa](?:nswer)?:\*\*|[Aa](?:nswer)?:\s*)\s*(.+)/
      );
      if (qMatch && aMatch) {
  ...
  const handleImport = async () => {
    if (!selectedDeck) {
      showNotification({
        title: "Error",
        message: "Please select a deck first.",
        type: "error",
      });
      return;
    }
    ...
    showNotification({
      title: "Success",
      message: `Imported ${importedCount} cards successfully.`,
      type: "success",
    });
    setDrafts([]);
    setText("");
    ...
    <>
      ...
      <div className={BASE}>
        ...
        <p className={`${BASE}__subtitle`}>
          Paste your Markdown Q&A from NotebookLM or CSV data from Zotero. We
          will parse it into flashcards.
        </p>
        ...
        <div className={`${BASE}__form`}>
          ...
          <label className={`${BASE}__draft-label`}>
            Source Reference (Optional Zotero URI / NotebookLM Link)
          </label>
          <input
            type="text"
            className={`${BASE}__draft-input`}
        ...
        {drafts.length > 0 && (
          <div className={`${BASE}__drafts`}>
            ...
            {drafts.map((draft, i) => (
              <div key={i} className={`${BASE}__draft-card`}>
                ...
                <div className={`${BASE}__actions`}>
                  <Button variant="ghost" onClick={() => removeDraft(i)}>
                    Reject
                  </Button>
                </div>
              </div>
            ...
            <div className={`${BASE}__actions`}>
              <Button variant="default" onClick={() => setDrafts([])}>
                Clear All
              </Button>
              <Button variant="primary" onClick={handleImport}>
                Import Selected ({drafts.length})
              </Button>
            </div>
          </div>
        )}
