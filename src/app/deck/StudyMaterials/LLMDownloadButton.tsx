...
import { IconCheck, IconCloudDownload, IconFile } from "@tabler/icons-react";
...
export default function LLMDownloadButton({ deck }: LLMDownloadButtonProps) {
  ...
  const [selectedFileToImport, setSelectedFileToImport] = useState<
    string | null
  >(null);
  ...
  const handleSavePastedContent = async () => {
    ...
    try {
      ...
      if (data.success) {
        showNotification({
          title: "Success",
          message: `Saved ${fileName} to OneDrive!`,
          type: "success",
        });
  ...
  const handleSelectFolder = async () => {
    try {
      if (!isTauri()) {
        showNotification({
          title: "Desktop Only",
          message: "Folder syncing is only available in the Mac Desktop app.",
          type: "info",
        });
        return;
      }
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      if (dataNotebooks.error) {
        showNotification({
          title: "NotebookLM Disconnected",
          message: `Please run notebooklm-mcp-auth in terminal. Error: ${dataNotebooks.error}`,
          type: "error",
        });
      } else {
        const notebook = dataNotebooks.notebooks?.find(
          (n: any) =>
            n.title.replace("_", "-").includes(deck.name) ||
            deck.name.includes(n.title.replace("_", "-"))
        );

        if (notebook) {
          setActiveNotebookId(notebook.id);
          const resStudio = await fetch(
            `http://localhost:3001/api/studio_status/${notebook.id}`
          );
          const dataStudio = await resStudio.json();
          if (dataStudio.error) {
            showNotification({
              title: "Studio Status Error",
              message: dataStudio.error,
              type: "error",
            });
      ...
      showNotification({
        ...
        message: "Could not connect to local proxy server.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleOpenImportModal = async (fileName: string) => {
    ...
    try {
      if (!isTauri()) {
        showNotification({
          title: "Desktop Only",
          message: "Importing local files is only available on desktop.",
          type: "error",
        });
        return;
      }
      ...
      if (newDrafts.length === 0) {
        showNotification({
          ...
          message: "Could not find any Q&A formatted flashcards in this file.",
          type: "error",
        });
        return;
      }
  ...
  const handleAutoImportFlashcards = async (artifact: any) => {
    ...
    showNotification({
      ...
      message:
        "Asking NotebookLM to generate flashcards... (This takes about 10-15s)",
      type: "info",
    });
    ...
    try {
      const res = await fetch("http://localhost:3001/api/query", {
        ...
        body: JSON.stringify({
          ...
          query:
            "Create exactly 10 high-yield flashcards covering the most important concepts of this notebook. Format each flashcard strictly with 'Q: [question]' on one line and 'A: [answer]' on the next line. Do not use markdown bold or extra text.",
        }),
      });
      ...
      if (newDrafts.length === 0) {
        showNotification({
          ...
          message:
            "LLM responded but we couldn't parse the Q: and A: format. Try again.",
          type: "error",
        });
        return;
      }
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      showNotification({
        title: "Success",
        message: `Imported ${parsedFlashcards.length} flashcards to "${subdeckName}"!`,
        type: "success",
      });
  ...
  const handleUnsyncFile = async (materialId: string) => {
    try {
      ...
      showNotification({
        title: "Success",
        message: "File unsynced from deck.",
        type: "success",
      });
    } catch (err: any) {
      showNotification({ title: "Error", message: err.message, type: "error" });
  ...
  const handleReconnect = async () => {
    try {
      showNotification({
        title: "Reconnecting",
        message: "Refreshing NotebookLM connection...",
        type: "info",
      });
      ...
      if (data.success) {
        showNotification({
          title: "Connected",
          message: "NotebookLM auth refreshed successfully!",
          type: "success",
        });
      ...
      showNotification({
        title: "Failed",
        message:
          "Could not reconnect. Please run notebooklm-mcp-auth in your terminal.",
        type: "error",
      });
    }
  };
    ...
    try {
      ...
      for (const fileName of files) {
        ...
        if (type === "audio" || type === "video" || type === "ppt") {
          showNotification({
            title: "Uploading...",
            message: `Uploading ${fileName} to cloud...`,
            type: "info",
          });
          if (!isTauri())
            throw new Error("Cannot read local files from a web browser.");
          ...
          const {
            data: { publicUrl },
          } = supabase.storage.from("renal-review").getPublicUrl(cloudPath);
      ...
      if (newMaterials.length > 0) {
        ...
        showNotification({
          title: "Success",
          message: `Synced ${newMaterials.length} new file(s) to deck!`,
          type: "success",
        });
      } else {
        showNotification({
          title: "Up to Date",
          message: "All files in this folder are already synced.",
          type: "info",
        });
      }
      setOpened(false);
  ...
  return (
    <>
          ...
          {!folderPath ? (
            ...
            <>
              ...
              {loading ? (
                ...
                >
                  <Text size="sm" style={{ fontWeight: 600, color: "#b45309" }}>
                    No files found in folder!
                  </Text>
                  <Text
                    size="sm"
                    style={{ marginTop: "8px", color: "#92400e" }}
                  >
                    Please download your Study Guides, Briefing Documents, and
                    Audio Overviews from NotebookLM and place them in the folder
                    shown above.
                  </Text>
                </div>
              ) : (
                <>
                  ...
                  >
                    {files.map((file) => {
                      ...
                      const syncedMaterial = deck.studyMaterials?.find(
                        (m) => m.title === fileTitle
                      );
                          ...
                          <div style={{ display: "flex", gap: "4px" }}>
                            ...
                            {isSynced && syncedMaterial && (
                              <Button
                                ...
                                variant="subtle"
                                onClick={() =>
                                  handleUnsyncFile(syncedMaterial.id)
                                }
                              >
                                Unsync
                              </Button>
                          ...
                          >
                            ...
                            >
                              ...
                              <Text size="sm">
                                ...
                                {artifact.type === "report"
                                  ? artifact.report_content || "Report"
                                  : artifact.type}
                                )
                              </Text>
                            </div>
                            {isDownloaded ? (
                              ...
                              <Button
                                ...
                                variant="default"
                                onClick={() =>
                                  handleAutoImportFlashcards(artifact)
                                }
                              >
      ...
      >
        <Stack gap="md">
          <Text size="sm">
            ...
            <strong>{activeArtifact?.title}</strong> below.
          </Text>
          <Text size="xs" variant="dimmed">
            Your NotebookLM tab was just opened. Click "Copy" on the document
            there, and paste it here to automatically save it as a Markdown
            (.md) file to your OneDrive.
          </Text>
          ...
          >
            <Button variant="subtle" onClick={() => setPasteModalOpened(false)}>
              Cancel
            </Button>
            <Button
      ...
      <Modal
        ...
        title="Import to Flashcards"
      >
        <Stack gap="md">
          <Text size="sm">
            Found <strong>{parsedFlashcards.length}</strong> flashcards in{" "}
            <code>{selectedFileToImport}</code>.
          </Text>
          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            <label style={{ fontSize: "14px", fontWeight: 500 }}>
              Collection / Subdeck Name
            </label>
            <input
              type="text"
              value={subdeckName}
          ...
          <Text size="xs" variant="dimmed">
            This will create a new subdeck inside "{deck.name}" and add these
            flashcards to it. The original .md file will remain in your OneDrive
            folder.
          </Text>
          <div
            style={{
